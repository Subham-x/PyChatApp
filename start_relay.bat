...
echo ========================================
echo     PyChat RELAY Server Launcher
echo ========================================
echo.
...
echo.
echo ========================================
echo      Starting PyChat RELAY Server
echo ========================================
echo.
echo [IMPORTANT] Run this on a VPS with public IP
echo [NOTE] All users will connect to THIS server
echo.

REM Start the relay server
.venv\Scripts\python.exe relay_server.py

REM If server exits, pause so user can see any error messages
if errorlevel 1 (
    echo.
    echo [ERROR] Relay exited with error code: %errorlevel%
    pause
)
